'use client';

import { useCards, useDeleteCard } from '@/lib/cards';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

export default function CardsPage() {
  const router = useRouter();
  const { data, isLoading } = useCards();
  const deleteMutation = useDeleteCard();

  const handleDelete = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este cartão?')) {
      await deleteMutation.mutateAsync(id);
    }
  };

  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900">Cartões</h1>
          <div className="flex gap-2">
            <button
              onClick={() => router.push('/dashboard')}
              className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300"
            >
              Voltar
            </button>
            <Link
              href="/cards/new"
              className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
            >
              Novo Cartão
            </Link>
          </div>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow">
          <div className="p-4 border-b">
            <h2 className="text-lg font-semibold">Meus Cartões</h2>
          </div>
          <div className="divide-y">
            {isLoading && <div className="p-6 text-gray-500">Carregando...</div>}
            {!isLoading && data && data.length === 0 && (
              <div className="p-6 text-gray-500">Nenhum cartão cadastrado.</div>
            )}
            {!isLoading && data && data.map(card => (
              <div key={card.id} className="p-4 flex items-center justify-between">
                <div>
                  <p className="font-medium">{card.nickname}</p>
                  <p className="text-sm text-gray-500">{card.brand} • Limite: R$ {card.creditLimit?.toFixed?.(2) ?? card.creditLimit}</p>
                </div>
                <div className="flex items-center gap-2">
                  <Link
                    href={`/cards/${card.id}/edit`}
                    className="px-3 py-1.5 text-sm bg-gray-100 rounded hover:bg-gray-200"
                  >
                    Editar
                  </Link>
                  <button
                    onClick={() => handleDelete(card.id)}
                    className="px-3 py-1.5 text-sm bg-red-50 text-red-700 rounded hover:bg-red-100"
                  >
                    Excluir
                  </button>
                </div>
              </div>
            ))}
          </div>
        </div>
      </main>
    </div>
  );
}
